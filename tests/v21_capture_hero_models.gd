...
func _run() -> void:
	var output_rel: String = "res://artifacts/screenshots/v21_hero_models_iteration_a.png"
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--v21-shot="):
	...
	if output_rel.ends_with("v21_hero_models_final.png"):
		world.target_orbit_yaw += 24.0
		world.orbit_yaw = world.target_orbit_yaw
		for _i in range(16):
			world._process(0.033)
			world._update_camera(0.033)
			await process_frame
		_save(viewport, "res://artifacts/screenshots/v21_hero_models_final_wide.png")
		world.debug_advance_to_island(1)
		for _i in range(24):
			world._process(0.033)
			world._update_camera(0.033)
			await process_frame
		_save(viewport, "res://artifacts/screenshots/v21_hero_models_final_portal.png")
	print("V21 hero models screenshot saved: ", output_rel)
	quit(0)
