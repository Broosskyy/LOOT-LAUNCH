...
func _run() -> void:
	...
	for _i in range(24):
		...
		await process_frame
	_save(viewport, "res://artifacts/screenshots/v28_iteration_a_start_density.png")
	world.target_orbit_yaw += 5.0
	world.orbit_yaw = world.target_orbit_yaw
	for _i in range(14):
		world._process(0.033)
		world._update_camera(0.033)
		await process_frame
	_save(viewport, "res://artifacts/screenshots/v28_iteration_b_midground_density.png")
	for _i in range(8):
		world._process(0.033)
		world._update_camera(0.033)
		await process_frame
	_save(viewport, "res://artifacts/screenshots/v28_environment_density_final.png")
	print("V28 density screenshots saved")
	quit(0)
