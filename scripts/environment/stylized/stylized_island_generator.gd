...
static func build(
	...
	if quality_level == 0 and not playable:
		segments = 8
	var ring := _build_ring(radius, segments, island_index, variant, playable)
	_build_grass_top(root, ring, segments, variant, mats, mesh_fn)
	_build_cliffs(root, ring, segments, radius, thickness, playable, island_index, variant, mats, mesh_fn)
	if playable:
...
static func _build_ring(
	...
	for i in range(segments):
		var wave := sin(float(i) * wave_a + island_index) * 0.06 + cos(float(i) * wave_b) * 0.04
		var irregular := wave + rng.randf_range(-0.05, 0.05)
		if not playable:
			irregular *= 0.65
...
static func _build_grass_top(
	...
	var squash := _squash_for_variant(variant)
	for i in range(segments):
	...
	mesh_fn.call(root, top.commit(), mats.get("grass_main", mats.grass))
...
static func _build_cliffs(
	...
	var depth: float = radius * (0.58 if playable else 0.42) + rng.randf_range(-0.3, 0.5)
	var cliff := SurfaceTool.new()
	cliff.begin(Mesh.PRIMITIVE_TRIANGLES)
	var layer_count: int = 4 if playable else 2
	var y1: float = 0.0
	for layer in range(layer_count):
		...
		for i in range(segments):
			...
			var t_top := Vector3(cos(a) * ring[i] * scale0 * skew_a, y0, sin(a) * ring[i] * squash * scale0)
			var t1p := Vector3(cos(b) * ring[next] * scale0 * skew_b, y0, sin(b) * ring[next] * squash * scale0)
			var b0 := Vector3(cos(a) * ring[i] * scale1, y1, sin(a) * ring[i] * squash * scale1)
			var b1 := Vector3(cos(b) * ring[next] * scale1, y1, sin(b) * ring[next] * squash * scale1)
	...
	mesh_fn.call(root, cliff.commit(), mats.get("stone_dark", mats.rock_dark))
	var tip := Vector3(0.0, y1 - maxf(0.45, thickness * 0.28), 0.0)
	...
	for i in range(segments):
		...
		var a := TAU * float(i) / float(segments)
		var b := TAU * float(next) / float(segments)
		var a_bottom := Vector3(cos(a) * ring[i] * last_scale, y1, sin(a) * ring[i] * squash * last_scale)
		var b_bottom := Vector3(cos(b) * ring[next] * last_scale, y1, sin(b) * ring[next] * squash * last_scale)
	...
	mesh_fn.call(root, bottom.commit(), mats.get("stone_main", mats.rock))
	if playable:
		for i in range(5):
			var facet_angle := TAU * float(i) / 5.0 + rng.randf_range(-0.25, 0.25)
			var facet := PrismMesh.new()
			facet.size = Vector3(radius * 0.18, depth * rng.randf_range(0.22, 0.38), radius * 0.14)
			mesh_fn.call(
				root, facet,
				mats.get("stone_light", mats.rock) if i % 2 == 0 else mats.get("stone_dark", mats.rock_dark),
...
static func _scatter_top_details(
	...
	for i in range(6):
		...
		var edge_pos := Vector3(cos(angle) * radius * 0.86, 0.14, sin(angle) * radius * 0.72)
		var stone := PrismMesh.new()
		stone.size = Vector3(rng.randf_range(0.45, 0.9), rng.randf_range(0.28, 0.55), rng.randf_range(0.4, 0.75))
		mesh_fn.call(root, stone, mats.get("stone_main", mats.rock), edge_pos, Vector3.ONE,
