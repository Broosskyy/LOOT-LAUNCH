...
static func quality_profile(quality_level: int) -> Dictionary:
	...
	base["ambient_energy"] = [0.40, 0.47, 0.51][q]
	base["exposure"] = [0.79, 0.82, 0.85][q]
	base["glow_intensity"] = [0.0, 0.062, 0.078][q]
	base["glow_bloom"] = [0.0, 0.014, 0.018][q]
	base["glow_threshold"] = [2.0, 1.80, 1.76][q]
	base["fog_density"] = [0.0, 0.00076, 0.00098][q]
	base["fog_aerial"] = [0.0, 0.32, 0.46][q]
	base["sun_energy"] = [0.74, 0.82, 0.90][q]
...
static func apply(
	...
	if world != null:
		...
		VisualMaster.apply_world_meta(world)


static func _apply_sky(sky_material: ProceduralSkyMaterial, quality_level: int) -> void:
	sky_material.sky_top_color = Color("2890c4")
	sky_material.sky_horizon_color = Color("96d4ec")
	sky_material.ground_horizon_color = Color("d8f2fc")
	sky_material.ground_bottom_color = Color("7ab8d8")
	sky_material.sun_angle_max = 12.5 if quality_level >= 2 else 11.0
	sky_material.sun_curve = 0.022
	sky_material.sky_energy_multiplier = 1.03 if quality_level >= 2 else 1.0


static func _apply_environment(environment: Environment, profile: Dictionary, quality_level: int) -> void:
	environment.background_mode = Environment.BG_SKY
	environment.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	environment.ambient_light_color = Color("c8e0f4")
	...
	if quality_level >= 1:
		...
		environment.adjustment_saturation = 1.05 if quality_level >= 2 else 1.02
