...
## V23 — Deterministic Wolkengarten world composition SSOT.
## Route layout, vista islands, camera tuning, and ring arcs live here only.

enum IslandRole { HERO_START, PRIMARY_DESTINATION, ROUTE, PLAYABLE, HERO_LANDMARK, VISTA, MICRO }

const CAMERA_FOV := 53.0
const CAMERA_PITCH := 20.0
const CAMERA_LOOK_HEIGHT := 0.78
const CAMERA_LOOK_AHEAD := 4.0
const CAMERA_FOLLOW_DISTANCE := 9.1
const CAMERA_FOLLOW_HEIGHT := 4.05

const PLAYER_SPAWN_OFFSET := Vector3(-2.1, 0.0, 2.1)
const CANNON_OFFSET := Vector3(0.15, 0.92, -2.45)
...
const VISTA_ISLANDS: Array[Dictionary] = [
	{"center": Vector3(-22.0, 16.0, -34.0), "radius": 5.0, "thickness": 0.82, "role": IslandRole.VISTA, "index": 20, "landmark": false},
	{"center": Vector3(18.0, 13.0, -42.0), "radius": 4.6, "thickness": 0.78, "role": IslandRole.VISTA, "index": 21, "landmark": false},
	{"center": Vector3(30.0, 32.0, -82.0), "radius": 7.8, "thickness": 0.95, "role": IslandRole.HERO_LANDMARK, "index": 22, "landmark": true},
	{"center": Vector3(-10.0, 34.0, -98.0), "radius": 4.8, "thickness": 0.8, "role": IslandRole.VISTA, "index": 33, "landmark": false},
	{"center": Vector3(28.0, 46.0, -148.0), "radius": 3.4, "thickness": 0.7, "role": IslandRole.MICRO, "index": 34, "landmark": false},
	{"center": Vector3(-26.0, 40.0, -122.0), "radius": 3.0, "thickness": 0.68, "role": IslandRole.MICRO, "index": 35, "landmark": false},
]
...
static func ring_arc_for_hop(route_index: int, from_center: Vector3, to_center: Vector3) -> Array[Vector3]:
	...
	for i in range(count):
		...
		var along: float = 0.68 if route_index == 0 else 0.62
		var pos: Vector3 = from_center.lerp(to_center, t * along)
		var apex: float = 3.2 if route_index == 0 else 2.4
		pos.y = lerpf(from_center.y + 2.2, to_center.y + 1.4, t) + sin(t * PI) * apex
		var side: float = 1.4 if route_index == 0 else 0.9
		pos.x += sin(t * PI * 1.35) * side
		points.append(pos)
	return points
...
static func composition_markers(world: Node) -> Dictionary:
	...
	if world.route_centers.size() > 1:
		markers["primary_destination"] = Vector3(world.route_centers[1]) + Vector3(0.0, 1.4, 0.0)
